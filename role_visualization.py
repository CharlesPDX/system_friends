...
from metacognitive import RoutingResult
from orchestrator import AssignmentResult
...
def create_role_pipeline_graph(
    ...
    for node in all_nodes:
        ...
        msv_summaries.append(
            ...
            f"CE={msv.correctness_evaluation.calculated_value}, "
            f"EM={msv.experiential_matching.calculated_value}, "
            f"CI={msv.conflicting_information.calculated_value}, "
...
def create_msv_evolution_chart(
    ...
    ce_vals = [float(r.node_msv.correctness_evaluation.calculated_value) for r in chain]
    em_vals = [float(r.node_msv.experiential_matching.calculated_value) for r in chain]
    ci_vals = [
        float(r.node_msv.conflicting_information.calculated_value) for r in chain
    ]
    ...
    for label, vals, color in dims:
        ...
        p.line(
            x="x", y="y", source=source, line_width=2, color=color, legend_label=label
        )
...
def create_routing_gauge(
    ...
    """
    methods = ["Paper (Eq 8)"]
    activations = [paper_activation]
    thresholds = [paper_threshold]
    colors = [
...
def create_role_pipeline_components(
    ...
    if routing_result:
        gauge = create_routing_gauge(
            routing_result.activation_value,
            routing_result.activation_threshold,
        )
