...
def get_git_revision_hash() -> dict:
    ...
    hashes = {
        ...
        "git_revision_short_hash": git_revision_short_hash,
    }
    return hashes


def create_database_and_table(
    db_file: str, configuration: dict[str, dict[str, Any]]
) -> bool:
    ...
    try:
        ...
        cursor.execute(
            """INSERT INTO parameters(datetime, parameters) VALUES (?, ?)""",
            (datetime.now(timezone.utc).isoformat(), json.dumps(configuration)),
        )
        conn.commit()
...
def record_interaction(
    ...
    system_two_msv: MetacognitiveVector | None,
) -> None:
    try:
        ...
        """,
            (
                ...
                system_one_response,
                system_two_response,
                json.dumps(system_one_msv.model_dump()),
                (
                    json.dumps(system_two_msv.model_dump())
                    if system_two_msv is not None
                    else None
                ),
            ),
        )
